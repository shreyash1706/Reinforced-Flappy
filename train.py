...
def train():
    ...
    # Plot Optimality on the left Y-axis (Red)
    ax1.set_xlabel('Episodes')
    ax1.set_ylabel('Optimality Rate (%)', color='tab:red')
    ax1.plot(optimality_history, color='tab:red', label='Optimality %')
    ax1.tick_params(axis='y', labelcolor='tab:red')

    # Plot Loss on the right Y-axis (Blue)
    ax2 = ax1.twinx()  
    ax2.set_ylabel('Average Loss', color='tab:blue')  
    ax2.plot(loss_history, color='tab:blue', alpha=0.5, label='Loss')
    ax2.tick_params(axis='y', labelcolor='tab:blue')

    plt.title('DQN Flappy Bird: Training Progress')
    fig.tight_layout()
    plt.savefig('learning_curve.png') # Saves the graph as an image in your folder
